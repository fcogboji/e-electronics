...
interface LaptopsCarouselProps {
  products: Product[];
}

const LaptopsCarousel = ({ products }: LaptopsCarouselProps) => {
  ...
  return (
    <div className="py-16 bg-gradient-to-br from-slate-50 via-gray-50 to-zinc-50">
      <div className="container mx-auto px-4">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-slate-700 to-gray-900 bg-clip-text text-transparent">
              Laptops & Computers
            </h2>
          </div>
          <p className="text-lg text-gray-600">Power Your Productivity</p>
          <div className="w-24 h-1 bg-gradient-to-r from-slate-600 to-gray-800 mx-auto rounded-full mt-4" />
        </motion.div>
              ...
              >
                <Link href={`/product/${product.id}`}>
                  <motion.div
                    ...
                    className="relative bg-white border border-slate-200 rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300 hover:border-slate-400 h-[350px]"
                  >
                    {/* Laptop Badge */}
                    <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-r from-slate-700 to-gray-900 text-white text-xs font-bold text-center py-1.5 flex items-center justify-center gap-1">
                    ...
                    {product.discount && product.discount > 0 && (
                      <div className="absolute top-10 left-0 z-10 bg-green-500 text-white px-2 py-1 text-xs font-bold">
                        -{product.discount}%
                      </div>
                    )}
                    ...
                    >
                      <Heart className={`w-4 h-4 transition-colors duration-300 ${
                        wishlistItems[product.id]?.isLiked ? 'fill-red-500 text-red-500' : 'text-gray-600 hover:text-slate-700'
                      }`} />
                    </motion.button>
